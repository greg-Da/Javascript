...
let x = 0
let res = 0;
while (nmb < journ.length) {
    x = 0
    while (x < journ[nmb].length) {
        if (journ[nmb][x] === "_") {
            res++;
        }
        x++;
...
console.log(journ.sort(new Intl.Collator('en').compare));

console.log(journ.sort(function (a, b) { return a.length - b.length }).slice(0, 50));
